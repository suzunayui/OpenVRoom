...
通信内容やVRM・ルームはサーバーに保存しません。新しい招待URLは `/room/12文字コード/` です。`deploy/setup-short-invites.sh` をsudoで実行すると既存TLS設定を保って直接アクセス用ルートを追加します。招待ルートのアクセスログを無効化し、Referrer-Policyをno-referrerにしてコードが参照元として送られないようにします。旧フラグメント形式と32文字コードも読み取れます。新コードは暗号学的乱数72ビットで生成し、既存コードとの重複を検査します。Signaling経路のアクセスログとTURNの接続ログは無効です。TURN資格情報は入室者にのみ発行し、8時間で失効します。ルームは最大6時間です。直接接続が可能な場合はWebRTCで直接通信し、失敗時に暗号化された通信をTURN経由で中継します。TURNはTLS/443待受を提供しないため、厳しい企業ネットワーク等で接続できない場合があります。
